    ...
    namespace NRE {
        namespace Maths {
            ...
            template <class T>
            Plane<T>::Plane(Plane && p) : n(std::move(p.getNormal())), p(std::move(p.getPoint())) {
            }

            template <class T>
            template <class K>
            Plane<T>::Plane(Plane<K> const& p) : n(static_cast <Vector3D<T>> (p.getNormal())), p(static_cast <Point3D<T>> (p.getPoint())) {
            }
